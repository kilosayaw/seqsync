...
const CoordinateGridEditor = ({ initialVector, onVectorChange }) => {
    ...
    const handleCellClick = (x, y) => {
        ...
        if (vector.x === x && vector.y === y) {
            // Clicked the same cell, cycle Z
            const nextZ = vector.z === 1 ? -1 : vector.z + 1; // Cycles 1 -> -1 -> 0 -> 1
            newVector = { ...vector, z: nextZ };
        } else {
            // Clicked a new cell, set new X/Y and reset Z to 0
            newVector = { x, y, z: 0 };
        }
        setVector(newVector);
    ...
    const gridCells = [];
    // Y is inverted for typical screen coordinates (top-to-bottom)
    for (let y of [1, 0, -1]) {
        for (let x of [-1, 0, 1]) {
            gridCells.push(
