...
import { faMicrochip, faTimes, faEye, faEyeSlash, faArrowsSpin } from '@fortawesome/free-solid-svg-icons'; // Added faArrowsSpin
import {
...
const StepSequencerControls = () => {
  ...
  const [weightShiftCurve, setWeightShiftCurve] = useState(50);
  ...
  const visualPoseRef = useRef(null);
 ...
 const { isAnalyzing, analysisProgress, startAnalysis, cancelAnalysis } = useMotionAnalysis({
   ...
   onKeyframeData: (keyframeData) => {
     ...
     setSongData(prevSongData => {
         ...
         // First, wipe all existing pose data to ensure a clean slate
         newSongData.forEach(bar => {
             bar.beats.forEach(beat => {
                 beat.jointInfo = {};
                 beat.grounding = { L: null, R: null, L_weight: 50 };
                 beat.headOver = null;
                 beat.thumbnail = null; // Also clear any old thumbnails
             });
         });
         // Now, apply the new keyframe data
         keyframeData.forEach(frame => {
             ...
             if (newSongData[bar] && newSongData[bar].beats[beat]) {
                 ...
                 targetBeat.thumbnail = thumbnail; // Assign the new thumbnail
             }
         });
         toast.success("Motion analysis complete. Sequence populated with new poses.");
         return newSongData;
     });
   },
   logDebug,
  ...
  const coreHandleClearPoseData = useCallback((barIdx, beatIdx) => {
    setSongData(prev => {
      const newData = makeDeepCopy(prev);
      const beat = newData[barIdx]?.beats?.[beatIdx];
      if (beat) {
        beat.jointInfo = {};
        beat.grounding = { L: null, R: null, L_weight: 50 };
        beat.thumbnail = null;
    ...
    toast.info(`Pose data cleared from Bar ${barIdx + 1}, Beat ${beatIdx + 1}.`);
  }, [makeDeepCopy]);
  ...
  }, [currentEditingBar, makeDeepCopy, uiPadsToRender]);
  ...
  const coreHandleRemoveBar = useCallback(() => {
    ...
    setSongData(prevSongData => {
      const newSongData = prevSongData.slice(0, -1);
  ...
  }, [totalBarsInSequence, currentEditingBar, coreHandleClearBar]);
  ...
  const handleJointDataUpdate = useCallback((jointAbbrev, dataType, value) => {
    setSongData(prev => {
      const newData = makeDeepCopy(prev);
      const beat = newData[currentEditingBar].beats[activeBeatIndex];
 ...
 const handleLoadSequence = useCallback((file) => {
   ...
   reader.onload = (e) => {
       try {
           ...
           setSongData(loadedData.songData);
 ...
 const coreHandlePasteBar = useCallback(() => {
   ...
   setSongData(prev => {
     const newData = makeDeepCopy(prev);
     newData[currentEditingBar].beats = makeDeepCopy(copiedBarData);
     return newData;
   });
   toast.success(`Pasted to Bar ${currentEditingBar + 1}.`);
 }, [currentEditingBar, copiedBarData, makeDeepCopy]);
 ...
 const coreHandlePastePoseData = useCallback(() => {
   ...
   setSongData(prev => {
     const newData = makeDeepCopy(prev);
     const targetBeat = newData[currentEditingBar].beats[activeBeatIndex];
 ...
 }, [currentEditingBar, activeBeatIndex, copiedPoseData, makeDeepCopy]);
 ...
 const coreHandleNavigateEditingBar = useCallback((direction) => {
  ...
  const coreHandleTapTempo = useCallback(() => {
   ...
   // Clear any previous timer to reset the timeout window
   if (tapTempoDataRef.current.timer) {
     clearTimeout(tapTempoDataRef.current.timer);
   }
   ...
   const coreHandleClearPoseData = useCallback((barIdx, beatIdx) => {
    setSongData(prev => {
      const newData = makeDeepCopy(prev);
      const beat = newData[barIdx].beats[beatIdx];
      beat.jointInfo = {}; // Clear the joint data
      beat.grounding = { L: null, R: null, L_weight: 50 }; // Reset grounding
      beat.thumbnail = null; // Clear thumbnail
      return newData;
    });
    toast.info(`Pose data cleared from beat ${beatIdx + 1}.`);
  }, [makeDeepCopy]);

   // Set a timer. If the user doesn't tap again within the max interval, process the taps.
   tapTempoDataRef.current.timer = setTimeout(() => {
     ...
     if (ts.length >= TAP_TEMPO_MIN_TAPS) {
       ...
       // Basic outlier removal: useful if user accidentally double-taps or waits too long between taps.
       ...
       if (intervals.length >= TAP_TEMPO_MIN_TAPS - 1) {
           ...
           if (avgInterval > 50 && avgInterval < TAP_TEMPO_MAX_INTERVAL_MS) {
               ...
               logDebug('info', `CoreAction: TapTempo - Calculated BPM: ${newBPM} from avg interval: ${avgInterval.toFixed(2)}ms`);
               coreHandleBpmChange(String(newBPM)); // Use the main BPM handler
               toast.info(`Tempo updated to: ~${newBPM} BPM`);
           } else {
               toast.warn("Tap tempo intervals too erratic or out of range.");
               logDebug('warn', `TapTempo avg interval out of range: ${avgInterval}`);
           }
       } else {
           toast.warn(`Not enough consistent taps (Need ${TAP_TEMPO_MIN_TAPS}).`);
           logDebug('warn', `TapTempo: Not enough valid intervals: ${intervals.length}`);
       }
     } else if (ts.length > 0) {
     ...
     // Reset for the next tap sequence
     tapTempoDataRef.current.timestamps = [];
     tapTempoDataRef.current.timer = null;
   }, TAP_TEMPO_MAX_INTERVAL_MS);


   // Prevent the timestamps array from growing indefinitely
   if (currentTaps.length > TAP_TEMPO_MIN_TAPS + 10) {
       tapTempoDataRef.current.timestamps = currentTaps.slice(-(TAP_TEMPO_MIN_TAPS + 5));
   }
 }, [coreHandleBpmChange, logDebug]); // Dependency on coreHandleBpmChange is correct

  const coreHandleAddBar = useCallback(() => {
   logDebug('info', `CoreAction: Add Bar. Current total: ${totalBarsInSequence}`);
   setSongData(prevSongData => {
     const newSongData = makeDeepCopy(prevSongData);
     const newBarId = newSongData.length; // 0-indexed ID for the new bar


     // Create a new bar object with the correct number of default beat objects
     newSongData.push({
       id: newBarId,
       beats: Array(uiPadsToRender).fill(null).map((_, i) => createDefaultBeatObject(i))
     });

     // Automatically focus the UI on the new bar for a better user experience
     setCurrentEditingBar(newBarId);
     setActiveBeatIndex(0);

     logDebug('info', `Bar Added. New total: ${newSongData.length}. Now editing Bar ${newBarId + 1}.`);
     return newSongData;
   });
   toast.success("New bar added.");
 }, [makeDeepCopy, uiPadsToRender, totalBarsInSequence, logDebug]);

  const coreHandleToggleLoop = useCallback(() => {
   const newIsLooping = !isLoopingRef.current;
   setIsLooping(newIsLooping); // Update state for re-render
   logDebug('info', `CoreAction: ToggleLoop - Loop is now ${newIsLooping ? 'ON' : 'OFF'}.`);


   if (!newIsLooping) {
     // Clear loop points when turning looping off
     setLoopStartPoint(null);
     setLoopEndPoint(null);
     toast.info("Looping OFF.");
   } else {
     // If turning loop ON and no points are set, default to the current editing bar
     if (!loopStartPointRef.current || !loopEndPointRef.current) {
       ...
       const defaultEnd = { bar: barForDefaultLoop, beat: Math.max(0, beatsInBar - 1) };
      
       setLoopStartPoint(defaultStart);
       setLoopEndPoint(defaultEnd);
      
       toast.success(`Looping ON for Bar ${barForDefaultLoop + 1}.`);
       logDebug('debug', `CoreAction: ToggleLoop - Defaulted loop to Bar ${barForDefaultLoop + 1}`);
     } else {
       toast.success("Looping ON.");
       logDebug('debug', `CoreAction: ToggleLoop - Loop ON with existing points.`);
     }
   }
 }, [currentEditingBar, uiPadsToRender, isLooping, songDataRef, setLoopStartPoint, setLoopEndPoint, logDebug]); // isLooping and setters are stable
  ...
  const coreHandleSetLoopPoint = useCallback((type) => { // type is 'start' or 'end'
   ...
   logDebug('info', `CoreAction: SetLoopPoint - Type: '${type}', Point: B${barToSet+1}:S${beatToSet+1}`);


   if (type === 'start') {
     setLoopStartPoint(pointToSet);
     // If the new start point is after the current end point, adjust the end point.
     if (!loopEndPointRef.current || pointToSet.bar > loopEndPointRef.current.bar || (pointToSet.bar === loopEndPointRef.current.bar && pointToSet.beat > loopEndPointRef.current.beat)) {
       ...
       const newEndPoint = { bar: pointToSet.bar, beat: Math.max(pointToSet.beat, beatsInBar - 1) };
       setLoopEndPoint(newEndPoint);
     ...
     // If the new end point is before the current start point, adjust the start point.
     if (!loopStartPointRef.current || pointToSet.bar < loopStartPointRef.current.bar || (pointToSet.bar === loopStartPointRef.current.bar && pointToSet.beat < loopStartPointRef.current.beat)) {
       const newStartPoint = { bar: pointToSet.bar, beat: 0 };
       setLoopStartPoint(newStartPoint);
   ...
   // Automatically turn on looping when setting a point
   if (!isLoopingRef.current) {
     setIsLooping(true);
     logDebug('debug', 'Looping turned ON as a loop point was set.');
   }
 }, [currentEditingBar, activeBeatIndex, uiPadsToRender, logDebug]);


  const coreHandleSkip = useCallback((direction) => { // direction: 1 for forward, -1 for backward
   logDebug('info', `CoreAction: Skip - Direction: ${direction > 0 ? 'Forward' : 'Backward'}, Interval: 1/${skipIntervalDenominator}`);
   const wasPlaying = isPlayingRef.current;
   if (wasPlaying) {
     setIsPlaying(false); // Pause playback for the skip operation
   }


   let numStepsToSkip;
   if (skipIntervalDenominator === 1) { // Special case for skipping by "Bar"
       if (direction > 0) { // Forward
           numStepsToSkip = uiPadsToRender - currentStepRef.current;
           if(numStepsToSkip === 0) numStepsToSkip = uiPadsToRender; // If at start of bar, jump a full bar
       } else { // Backward
           numStepsToSkip = currentStepRef.current + 1;
           if(currentStepRef.current === 0) numStepsToSkip = uiPadsToRender; // If at start of bar, jump back to start of previous
       }
   } else {
   ...
   logDebug('info', `Skip: Setting new position - Bar:${newBar + 1}, Beat:${newStep + 1}`);
  
   // Update state for both playback and UI
   setCurrentBar(newBar);
   setCurrentStep(newStep);
   setCurrentEditingBar(newBar);
   setActiveBeatIndex(newStep);
   ...
   // Sync the video player if it exists
   if (videoPlayerRef.current) {
       const timePerStep = (60 / bpmRef.current) / (uiPadsToRender / musicalBeatsPerBar);
       const newTime = (newBar * uiPadsToRender + newStep) * timePerStep;
       if (isFinite(newTime) && newTime <= videoPlayerRef.current.duration) {
           videoPlayerRef.current.currentTime = newTime;
       }
   }
      
   if (wasPlaying) {
     // Use a brief timeout to allow state to update before resuming playback
     setTimeout(() => setIsPlaying(true), 50);
   }
 }, [skipIntervalDenominator, uiPadsToRender, totalBarsInSequence, musicalBeatsPerBar, logDebug]);

   const coreHandleTimeSignatureChange = useCallback((newTimeSignature) => {
   logDebug('info', `CoreAction: TimeSignatureChange - New: ${newTimeSignature.beatsPerBar}/${newTimeSignature.beatUnit}`);
   if (setContextTimeSignatureLocal) {
     setContextTimeSignatureLocal(newTimeSignature);
   }
   toast.info(`Time Signature set to ${newTimeSignature.beatsPerBar}/${newTimeSignature.beatUnit}. Playback timing will adjust.`);
 }, [setContextTimeSignatureLocal, logDebug]);

  const coreHandleMediaOriginalBPMChange = useCallback((newBpmVal) => {
   // This handler updates the state for the input field where the user can
   // manually enter the BPM of the loaded media file.
   setMediaOriginalBPM(newBpmVal);
   logDebug('info', `CoreAction: MediaOriginalBPMChange - Value set to: '${newBpmVal}'`);
 }, []);


  const coreHandleDetectMediaBPM = useCallback(async () => {
   logDebug('info', 'CoreAction: DetectMediaBPM - Clicked.');
   toast.info("BPM Detection from media is a planned feature and is not yet implemented.");
  
   // --- FUTURE IMPLEMENTATION NOTES (Phase 2) ---
   // This is where you would integrate an audio processing library like aubio.js.
   // The process would be:
   // 1. Check if a media file (video or audio) is loaded.
   // if (!loadedMediaFile) {
   //   toast.error("No media file loaded to detect BPM from.");
   //   return;
   // }
   // 2. Extract the audio data from the media file into an AudioBuffer.
   //    This might involve using an offline audio context.
   // 3. Pass the AudioBuffer to the BPM detection algorithm.
   //    const detectedBpm = await runBpmDetection(audioBuffer);
   // 4. Update the state with the result.
   //    if (detectedBpm) {
   //      setMediaOriginalBPM(String(Math.round(detectedBpm)));
   //      toast.success(`Detected Media BPM: ~${Math.round(detectedBpm)}`);
   //    } else {
   //      toast.warn("Could not detect BPM from the media.");
   //    }
 }, []);
  ...
  const coreHandleSkipIntervalChange = useCallback((value) => {
   // The value from the Select component is usually a string, so we parse it.
   const newDenominator = parseInt(value, 10);
  
   // Validate that the new value is a number and is one of the allowed options.
   if (!isNaN(newDenominator) && SKIP_OPTIONS.some(opt => parseInt(opt.value, 10) === newDenominator)) {
       ...
       logDebug('info', `CoreAction: SkipIntervalChange - Denominator set to: 1/${newDenominator}`);
   } else {
       logDebug('warn', `CoreAction: SkipIntervalChange - Invalid skip interval value received: '${value}'`);
   }
 }, []); // No external dependencies are needed for this handler.

  const coreHandleToggleShift = useCallback(() => {
   setIsShiftActive(prevIsShiftActive => {
     const nextState = !prevIsShiftActive;
     logDebug('info', `CoreAction: ToggleShift - Shift state changed from ${prevIsShiftActive} to ${nextState}`);
     return nextState;
   });
 }, []); // No external dependencies are needed for this handler.
  ...
  const wrappedPlaybackHandlers = useMemo(() => ({
    onPlayPause: coreHandlePlayPause,
    onStop: coreHandleStop,
    onScrub: coreHandleScrub,
    onTapTempo: coreHandleTapTempo,
    onNextBar: () => coreHandleNavigateEditingBar(1),
    onPrevBar: () => coreHandleNavigateEditingBar(-1),
    onAddBar: coreHandleAddBar,
    onRemoveBar: coreHandleRemoveBar,
    onClearBar: coreHandleClearBar,
    onToggleLoop: coreHandleToggleLoop,
    onSetLoopStart: () => coreHandleSetLoopPoint('start'),
    onSetLoopEnd: () => coreHandleSetLoopPoint('end'),
    onCopyBar: coreHandleCopyBar,
    onPasteBar: coreHandlePasteBar,
    canPaste: !!copiedBarData,
    onCopyPose: coreHandleCopyPoseData,
    onPastePose: coreHandlePastePoseData,
    canPastePose: !!copiedPoseData,
    onSkipForward: () => coreHandleSkip(1),
    onSkipBackward: () => coreHandleSkip(-1),
    onDetectMediaBPM: coreHandleDetectMediaBPM,
    onSetDetectedBPMAsMaster: coreHandleSetDetectedBPMAsMaster,
    onTimeSignatureChange: coreHandleTimeSignatureChange,
    onMediaOriginalBPMChange: coreHandleMediaOriginalBPMChange,
    onSkipIntervalChange: coreHandleSkipIntervalChange,
    onToggleShift: coreHandleToggleShift,
    onBpmChange: handleBpmChangeForInput,
  }), [
    // Ensure all dependencies are here, especially handleBpmChangeForInput
  ...
  // Animation Logic
  useEffect(() => {
    if (activePoseData !== targetPose) {
      visualPoseRef.current = targetPose;
      setTargetPose(activePoseData);
      setLerpAmount(0);
    }
  }, [activePoseData, targetPose]);

  useEffect(() => {
    let animationFrame;
    if (lerpAmount < 1) {
      animationFrame = requestAnimationFrame(() => {
        setLerpAmount(prev => Math.min(prev + 0.05, 1));
      });
    }
  ...
  useEffect(() => {
    // This effect handles audio playback, switching between video-driven and interval-driven timing.
    if (mediaSrcUrl && videoPlayerRef.current) {
      ...
      const handleVideoEnded = () => {
        if (isLoopingRef.current) {
            videoElement.currentTime = 0; // Or a specific loop start time in seconds
            videoElement.play();
        } else {
            setIsPlaying(false);
        }
  ...
  }, [activeEditingJoint, currentEditingBar, uiPadsToRender, currentSelectedKitObject, coreHandleMainUIGroundingChange]);
  ...
  // RENDER BLOCK
  const leftIndicatorOpacity = Math.max(0.2, (100 - crossfaderValue) / 50 * 0.8 + 0.2);
  const rightIndicatorOpacity = Math.max(0.2, crossfaderValue / 50 * 0.8 + 0.2);
  const leftIndicatorGlow = `0 0 15px rgba(59, 130, 246, ${Math.max(0, (80 - crossfaderValue) / 30)})`;
  const rightIndicatorGlow = `0 0 15px rgba(239, 68, 68, ${Math.max(0, (crossfaderValue - 20) / 30)})`;
          ...
          <div ref={centralColumnRef} className="order-2 flex flex-col items-center justify-start gap-2 overflow-hidden">
            ...
            <div className="w-full bg-gray-800/30 p-3 rounded-lg flex flex-col items-center justify-center flex-shrink-0">
              <div className="grid grid-cols-8 gap-2 mb-2 w-full">
                {currentBarBeatsForUI.slice(0, 8).map((beat, beatIdx) => ( <BeatButton key={`beat-A-${beatIdx}`} {...{ barIndex: currentEditingBar, beatIndex: beatIdx, isActive: activeBeatIndex === beatIdx, isCurrentStep: isPlaying && currentBar === currentEditingBar && currentStep === beatIdx, onClick: coreHandleBeatClick, sounds: beat?.sounds, viewMode, hasPoseData: !!beat?.jointInfo && Object.keys(beat.jointInfo).length > 0, poseData: beat?.jointInfo, grounding: beat?.grounding, thumbnail: beat?.thumbnail, currentSoundInBank, onAddSound: coreHandleAddSoundToBeat, onDeleteSound: coreHandleDeleteSound, onClearPoseData: coreHandleClearPoseData }}/> ))}
              </div>
              ...
              <div className="flex items-center justify-around w-full h-full">
                ...
                <div className="flex flex-col items-center justify-center gap-2 h-full">
                  ...
                  <div className="relative w-full max-w-[280px] aspect-video bg-black rounded-lg shadow-lg flex items-center justify-center flex-grow min-h-0">
                    {showCoreVisualizer ? (
                       <CoreDynamicsVisualizer
                          fullPoseState={targetPose?.jointInfo || {}}
                          onCoreInputChange={handleJointDataUpdate}
                          beatGrounding={targetPose?.grounding || { L: null, R: null}}
                          className="w-full h-full"
                        />
                    ) : (
                      <>
                        {mediaSrcUrl ? <VideoMediaPlayer ref={videoPlayerRef} mediaSrc={mediaSrcUrl} /> : 
                          <P5SkeletalVisualizer
                            fullPoseState={visualPoseRef.current?.jointInfo || {}}
                    ...
                    {/* This div contains the toggle buttons overlaid on the top-right of the visualizer */}
                  ...
                  {/* [FIXED] This div is now correctly placed *after* the visualizer container, ensuring it is always visible. */}
                  <div className="flex flex-row items-center gap-4 flex-shrink-0 mt-2">
                      <Crossfader value={crossfaderValue} onChange={setCrossfaderValue} className="w-40 h-4"/>
                      <RotationKnob value={weightShiftCurve} onChange={setWeightShiftCurve} size={48} label="Curve" />
                  </div>
