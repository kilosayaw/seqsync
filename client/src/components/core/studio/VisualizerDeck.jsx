...
import StabilityMeter from '../../common/StabilityMeter'; // <-- IMPORT THE NEW COMPONENT
...
const VisualizerDeck = () => {
    ...
    // `analyzePoseDynamics` now returns stability along with everything else
    const poseDynamics = useMemo(() => analyzePoseDynamics(targetPose), [targetPose]);
    const { kineticPath, centerOfMass, stability } = poseDynamics;
    ...
    // Callbacks and Handlers (no changes here from last time)
    const handleFaderChange = useCallback((newValue) => {
        if (faderMode === 'WEIGHT') {
            ...
            updateBeatDynamics(currentEditingBar, activeBeatIndex, {
                grounding: { L_weight: newWeight }
            });
        } else if (faderMode === 'HEAD') {
            const newHeadX = (newValue / 50) - 1;
            updateBeatDynamics(currentEditingBar, activeBeatIndex, {
                jointInfo: { H: { vector: { x: newHeadX } } }
            });
        }
