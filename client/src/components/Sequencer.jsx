...
import DebugPanel from './common/DebugPanel.jsx';
...
const Sequencer = () => {
    // --- 1. Get all state and setters from contexts ---
    const { 
    ...
    // --- 3. Define Callbacks BEFORE they are used in hooks ---
    ...
    const onPoseUpdate = useCallback((pose) => {
                ...
                <main className="flex-grow flex justify-center items-center p-4 min-h-0">
                   <div className="flex items-start gap-4 w-full max-w-[1400px]">
                        ...
                        <section className="flex-grow flex flex-col items-center gap-4 min-w-0">
                            <VisualizerDeck poseData={isLiveCamActive ? livePoseData : activeBeatData?.pose} />
                            <NotationDisplay />
                            <BeatGrid onBeatSelect={handleBeatSelect} onAddSoundClick={(bar, beat) => { setBeatToAddTo({bar, beat}); setSoundBrowserOpen(true); }} onSoundDelete={(bar, beat, url) => removeSoundFromBeat(bar, beat, url)} />
                        </section>
                        <aside className="flex-shrink-0 flex flex-col items-center gap-4">
